...
from ThreeDDFA_utils.functions import plot_image
from ThreeDDFA_utils.io import _load
from ThreeDDFA_utils.tddfa_util import _to_ctype
...
def process_uv(uv_coords, uv_h=1024, uv_w=1024):
...
g_uv_coords = load_uv_coords(make_abs_path('../ThreeDDFA_configs/BFM_UV.mat'))
indices = _load(make_abs_path('../ThreeDDFA_configs/indices.npy'))  # todo: handle bfm_slim
...
def uv_tex(img, ver_lst, tri, uv_h=1024, uv_w=1024, uv_c=3, show_flag=False, wfp= None):
    ...
    # concat if there is more than one image
