...
def get_landmark(input_img, detector, predictor):
...
def get_alignment_positions(input_img, detector, predictor, eyes_distance_only: bool = True):
    lm = get_landmark(input_img, detector, predictor)
...
def crop_face_by_transform(input_img, quad: np.ndarray, qsize: int, output_size: int = 1024,
...
def align_face(input_img, detector, predictor):
    c, x, y = get_alignment_positions(input_img, detector, predictor)
    quad, qsize = get_alignment_transformation(c, x, y)
    img = crop_face_by_transform(input_img, quad, qsize)
    return img


def crop_face(input_img, detector, predictor, random_shift=0.0):
    c, x, y = get_alignment_positions(input_img, detector, predictor)
    if random_shift > 0:
...
def get_stylegan_transform(unaligned_img, aligned_img, detector, predictor):
    try:
        aligned_img_lm = get_landmark(aligned_img, detector, predictor)
        aligned_left_eye, aligned_right_eye = get_eyes_coors(aligned_img_lm)
        unaligned_img_lm = get_landmark(unaligned_img, detector, predictor)
        ...
        rotated_aligned_img_lm = get_landmark(unaligned_img, detector, predictor)
        ...
        print(f"Failed aligning image: check style gan function. Got exception: {e}")
        return None
